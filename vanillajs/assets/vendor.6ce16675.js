 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class pt extends oe{_renderMetric(e){const t=this.dom.createComponent("metric"),n=this.dom.find(".lh-metric",t);n.id=e.result.id;const o=h.calculateRating(e.result.score,e.result.scoreDisplayMode);n.classList.add(`lh-metric--${o}`);const i=this.dom.find(".lh-metric__title",t);i.textContent=e.result.title;const a=this.dom.find(".lh-metric__value",t);a.textContent=e.result.displayValue||"";const l=this.dom.find(".lh-metric__description",t);if(l.appendChild(this.dom.convertMarkdownLinkSnippets(e.result.description)),e.result.scoreDisplayMode==="error"){l.textContent="",a.textContent="Error!";const s=this.dom.createChildOf(l,"span");s.textContent=e.result.errorMessage||"Report error: no metric information"}return n}_renderOpportunity(e,t){const n=this.dom.createComponent("opportunity"),o=this.populateAuditValues(e,n);if(o.id=e.result.id,!e.result.details||e.result.scoreDisplayMode==="error")return o;const i=e.result.details;if(i.type!=="opportunity")return o;const a=this.dom.find("span.lh-audit__display-text, div.lh-audit__display-text",o),l=`${i.overallSavingsMs/t*100}%`;if(this.dom.find("div.lh-sparkline__bar",o).style.width=l,a.textContent=h.i18n.formatSeconds(i.overallSavingsMs,.01),e.result.displayValue){const s=e.result.displayValue;this.dom.find("div.lh-load-opportunity__sparkline",o).title=s,a.title=s}return o}_getWastedMs(e){if(e.result.details&&e.result.details.type==="opportunity"){const t=e.result.details;if(typeof t.overallSavingsMs!="number")throw new Error("non-opportunity details passed to _getWastedMs");return t.overallSavingsMs}else return Number.MIN_VALUE}_getScoringCalculatorHref(e){const t=e.filter(d=>d.group==="metrics"),n=e.find(d=>d.id==="first-cpu-idle"),o=e.find(d=>d.id==="first-meaningful-paint");n&&t.push(n),o&&t.push(o);const i=d=>Math.round(d*100)/100,l=[...t.map(d=>{let u;return typeof d.result.numericValue=="number"?(u=d.id==="cumulative-layout-shift"?i(d.result.numericValue):Math.round(d.result.numericValue),u=u.toString()):u="null",[d.acronym||d.id,u]})];h.reportJson&&(l.push(["device",h.reportJson.configSettings.formFactor]),l.push(["version",h.reportJson.lighthouseVersion]));const s=new URLSearchParams(l),c=new URL("https://googlechrome.github.io/lighthouse/scorecalc/");return c.hash=s.toString(),c.href}_classifyPerformanceAudit(e){return e.group?null:e.result.details&&e.result.details.type==="opportunity"?"load-opportunity":"diagnostic"}render(e,t,n){const o=h.i18n.strings,i=this.dom.createElement("div","lh-category");i.id=e.id,i.appendChild(this.renderCategoryHeader(e,t,n));const a=e.auditRefs.filter(m=>m.group==="metrics");if(a.length){const[m,w]=this.renderAuditGroup(t.metrics),_=this.dom.createElement("input","lh-metrics-toggle__input"),C=`lh-metrics-toggle${h.getUniqueSuffix()}`;_.setAttribute("aria-label","Toggle the display of metric descriptions"),_.type="checkbox",_.id=C,m.prepend(_);const y=this.dom.find(".lh-audit-group__header",m),k=this.dom.createChildOf(y,"label","lh-metrics-toggle__label");k.htmlFor=C;const A=this.dom.createChildOf(k,"span","lh-metrics-toggle__labeltext--show"),S=this.dom.createChildOf(k,"span","lh-metrics-toggle__labeltext--hide");A.textContent=h.i18n.strings.expandView,S.textContent=h.i18n.strings.collapseView;const E=this.dom.createElement("div","lh-metrics-container");m.insertBefore(E,w),a.forEach(H=>{E.appendChild(this._renderMetric(H))});const z=this.dom.find(".lh-category-header__description",i),T=this.dom.createChildOf(z,"div","lh-metrics__disclaimer"),$=this.dom.convertMarkdownLinkSnippets(o.varianceDisclaimer);T.appendChild($);const L=this.dom.createChildOf(T,"a","lh-calclink");L.target="_blank",L.textContent=o.calculatorLink,this.dom.safelySetHref(L,this._getScoringCalculatorHref(e.auditRefs)),m.classList.add("lh-audit-group--metrics"),i.appendChild(m)}const l=this.dom.createChildOf(i,"div","lh-filmstrip-container"),s=e.auditRefs.find(m=>m.id==="screenshot-thumbnails"),c=s&&s.result;if(c&&c.details){l.id=c.id;const m=this.detailsRenderer.render(c.details);m&&l.appendChild(m)}const d=e.auditRefs.filter(m=>this._classifyPerformanceAudit(m)==="load-opportunity").filter(m=>!h.showAsPassed(m.result)).sort((m,w)=>this._getWastedMs(w)-this._getWastedMs(m)),u=a.filter(m=>!!m.relevantAudits);if(u.length&&this.renderMetricAuditFilter(u,i),d.length){const m=2e3,w=d.map(E=>this._getWastedMs(E)),_=Math.max(...w),C=Math.max(Math.ceil(_/1e3)*1e3,m),[y,k]=this.renderAuditGroup(t["load-opportunities"]),A=this.dom.createComponent("opportunityHeader");this.dom.find(".lh-load-opportunity__col--one",A).textContent=o.opportunityResourceColumnLabel,this.dom.find(".lh-load-opportunity__col--two",A).textContent=o.opportunitySavingsColumnLabel;const S=this.dom.find(".lh-load-opportunity__header",A);y.insertBefore(S,k),d.forEach(E=>y.insertBefore(this._renderOpportunity(E,C),k)),y.classList.add("lh-audit-group--load-opportunities"),i.appendChild(y)}const p=e.auditRefs.filter(m=>this._classifyPerformanceAudit(m)==="diagnostic").filter(m=>!h.showAsPassed(m.result)).sort((m,w)=>{const _=m.result.scoreDisplayMode==="informative"?100:Number(m.result.score),C=w.result.scoreDisplayMode==="informative"?100:Number(w.result.score);return _-C});if(p.length){const[m,w]=this.renderAuditGroup(t.diagnostics);p.forEach(_=>m.insertBefore(this.renderAudit(_),w)),m.classList.add("lh-audit-group--diagnostics"),i.appendChild(m)}const g=e.auditRefs.filter(m=>this._classifyPerformanceAudit(m)&&h.showAsPassed(m.result));if(!g.length)return i;const f={auditRefs:g,groupDefinitions:t},b=this.renderClump("passed",f);i.appendChild(b);const v=[];if(["performance-budget","timing-budget"].forEach(m=>{const w=e.auditRefs.find(_=>_.id===m);if(w&&w.result.details){const _=this.detailsRenderer.render(w.result.details);_&&(_.id=m,_.classList.add("lh-details","lh-details--budget","lh-audit"),v.push(_))}}),v.length>0){const[m,w]=this.renderAuditGroup(t.budgets);v.forEach(_=>m.insertBefore(_,w)),m.classList.add("lh-audit-group--budgets"),i.appendChild(m)}return i}renderMetricAuditFilter(e,t){const n=this.dom.createElement("div","lh-metricfilter"),o=this.dom.createChildOf(n,"span","lh-metricfilter__text");o.textContent=h.i18n.strings.showRelevantAudits;const i=[{acronym:"All"},...e],a=h.getUniqueSuffix();for(const l of i){const s=`metric-${l.acronym}-${a}`,c=this.dom.createChildOf(n,"input","lh-metricfilter__radio");c.type="radio",c.name=`metricsfilter-${a}`,c.id=s;const d=this.dom.createChildOf(n,"label","lh-metricfilter__label");d.htmlFor=s,d.title=l.result&&l.result.title,d.textContent=l.acronym||l.id,l.acronym==="All"&&(c.checked=!0,d.classList.add("lh-metricfilter__label--active")),t.append(n),c.addEventListener("input",u=>{for(const g of t.querySelectorAll("label.lh-metricfilter__label"))g.classList.toggle("lh-metricfilter__label--active",g.htmlFor===s);t.classList.toggle("lh-category--filtered",l.acronym!=="All");for(const g of t.querySelectorAll("div.lh-audit")){if(l.acronym==="All"){g.hidden=!1;continue}g.hidden=!0,l.relevantAudits&&l.relevantAudits.includes(g.id)&&(g.hidden=!1)}const p=t.querySelectorAll("div.lh-audit-group, details.lh-audit-group");for(const g of p){g.hidden=!1;const f=Array.from(g.querySelectorAll("div.lh-audit")),b=!!f.length&&f.every(v=>v.hidden);g.hidden=b}})}}}/**
 * @license
 * Copyright 2018 The Lighthouse Authors. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
