describe('Vue Component', () => {
  ...
  it('Check performance metrics', () => {
    ...
    cy.get('#speed-index > .lh-metric__innerwrap > .lh-metric__value').contains('1.6 s');
    cy.get('#largest-contentful-paint > .lh-metric__innerwrap > .lh-metric__value').contains('1.2 s');
  ...
  it('Runtime Settings', () => {
    ...
    cy.get(':nth-child(8) > .lh-env__description').contains(
      'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4359.0 Safari/537.36',
    );
    cy.get(':nth-child(9) > .lh-env__description').contains(
      'Mozilla/5.0 (Linux; Android 7.0; Moto G (4)) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/84.0.4143.7 Mobile Safari/537.36 Chrome-Lighthouse',
    );
    cy.get(':nth-child(10) > .lh-env__description').contains('2578');
    cy.get('.lh-footer__version').contains('7.0.0');
    cy.get(':nth-child(11) > .lh-env__description').contains('4.1.1');
  });
});
